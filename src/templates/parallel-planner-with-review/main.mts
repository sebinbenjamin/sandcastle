...
//   Phase 1 (Plan):             The planner agent analyzes open issues, builds
//                               a dependency graph, and outputs a <plan> JSON
//                               listing unblocked issues with branch names.
//   Phase 2 (Execute + Review): For each issue, a sandbox is created via
//                               createSandbox(). The implementer runs first
//                               (100 iterations). If it produced commits that
//                               are not merged yet, a reviewer runs in the
...
// completion signal, already-merged branches are never re-implemented or
// re-merged, and Ctrl-C aborts cleanly with sandbox cleanup.
//
// Usage:
...
// per-provider planner model; implementation, review, and merge share the
// worker model.
...
// failures abort the loop; implementer and reviewer failures exclude their
// issue from the merge set instead.
...
// to createSandbox's runs below; Sandcastle's own shutdown registry then
// force-removes the containers. The signal is checked at phase boundaries
// too, so an aborted run never starts the next agent invocation.
...
try {
  ...
  for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
    ...
    // Issue branches the target already contains skip the pipeline — the
    ...
    // runs first; if it produces unmerged commits, the reviewer runs in the
    // same sandbox.
    //
    // Promise.allSettled means one failing pipeline doesn't cancel the others.
    // -------------------------------------------------------------------------

    // The pipeline returns the run result of the last completed stage (with
    // commits from both stages merged in), or null when a completion gate
    // excluded the issue from the merge set.
    type PipelineResult =
      | Awaited<
          ReturnType<
            Awaited<ReturnType<typeof sandcastle.createSandbox>>["run"]
          >
        >
      | null;

    const settled = await Promise.allSettled(
      openIssues.map(async (issue): Promise<PipelineResult> => {
        ...
        try {
          ...
          const implement = await sandbox.run({
            name: "implementer",
            // Give each agent plenty of room to implement and iterate on
            // tests.
            maxIterations: 100,
            signal: runSignal,
          ...
          // Completion gate: an implementer that never emitted its signal
          // didn't finish — treat it as failed and keep it out of the merge
          // set instead of merging partial work.
          if (implement.completionSignal === undefined) {
            console.error(
              `  ✗ ${issue.id} (${issue.branch}): implementer finished without its completion signal — excluding from the merge set.`,
            );
            return null;
          }

          // Only review branches that actually produced commits, and only
          // while there is something unmerged left to review.
          if (
            ...
            // A review that never signaled its completion is an unfinished
            // review — exclude the branch from this round's merge set.
            if (review.completionSignal === undefined) {
              console.error(
                `  ✗ ${issue.id} (${issue.branch}): reviewer finished without its completion signal — excluding from the merge set.`,
              );
              return null;
            }

            // Merge commits from both runs so the merge phase sees all of
    ...
    // An aborted signal rejects every in-flight pipeline — surface it
    // immediately instead of walking a partially-failed round.
    ...
    // Merge candidates: pipelines that fulfilled, were not excluded by a
    // completion gate, and produced commits.
    const implementedIssues = settled
      ...
      .filter(
        (
          ...
          outcome: PromiseFulfilledResult<Exclude<PipelineResult, null>>;
    ...
    // itself). Together with the completion gates this prevents duplicate
    // merges on re-runs.
